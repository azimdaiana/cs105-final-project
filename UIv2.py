...
import blessed
import time
from utilities import load_map, print_map, randomMaze, getCurrentLocation, setLocation, guard_found, generate_problem, loadSavedGame, savingGame, goalReached, resetCurrentLocation, canGoWest, canGoEast, canGoSouth, canGoNorth
...
def draw_map(progMap, player_x, player_y):
    """
    Draws the map with the player’s current position marked with 'Y'.
    """
...
def UI_run():
    ...
    with term.fullscreen(), term.cbreak(), term.hidden_cursor():
        ...
        while not win:
            ...
            if 'move' in i:
                direction = input(term.move(len(grid) + 6,
                                            2) + "> Which direction would you like to go? (north/south/east/west): ").lower()
                x, y = player_x, player_y

                # Adjust player coordinates based on the direction
                if direction == "east":
                ...
                # Update player coordinates after movement
                player_x, player_y = getCurrentLocation()
            ...
            elif "print" in i:  # cheat key to print map directly
                print(grid)
